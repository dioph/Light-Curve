...
def tendencia(win, arq, novo_arq, cbv_list, mask=None, data=None, xdim=None, ydim=None):
	...
	if mask is not None:
		...
		timemask = timedata[mask]
		fluxmask = flux_array[mask]
		cadmask = caddata[mask]
	...
	if data is not None:
		pw1, pw2 = plotfit(win, timedata, fluxdata, flux_array, soma_normal, cad, version, row=ydim+2, col=xdim+2, rowspan=int((ydim+2-(ydim%2))/2), colspan=xdim+2)
	else:
...
def plotfit(win, timedata, fluxdata1, fluxdata2, soma, cad, version, row=0, col=0, rowspan=1, colspan=1):
	...
	pw1.plot(x=timedata, y=fluxdata1, pen='b', connect=sub)
	pw1.plot(x=timedata, y=soma, pen='r', connect=sub)
	...
	pw2.plot(x=timedata, y=fluxdata2, pen='b', connect=sub)
	return pw1, pw2
